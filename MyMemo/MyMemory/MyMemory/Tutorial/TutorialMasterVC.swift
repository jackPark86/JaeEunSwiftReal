...
class TutorialMasterVC: UIViewController, UIPageViewControllerDataSource {
    ...
    override func viewDidLoad() {
        ...
        //아래 3단계 구문은 커승텀 컨테이너 뷰 컨트롤러를 설정하기 위해 항상 필요한 과정이다. (p860)
    ...
    //컨텐츠 뷰 컨트롤러를 동적으로 생성할 getContentsVC
    func getContentVC(atIndex idx: Int) -> UIViewController? {
        ...
        //instanceTutorialVC 메소드는 Utils 파일에 작성된 익스텐션을 통해 가져온다. (p.856)
    ...
    //페이지 인디케이터 구성
    //페이지 뷰 컨트롤러가 출력할 페이지의 개수  
    func presentationCount(for pageViewController: UIPageViewController) -> Int {
        return self.contentTitles.count
    }//end of presentationCount
    
    //맨 처음 선택되어 있을 페이지 정보
    func presentationIndex(for pageViewController: UIPageViewController) -> Int {
        return 0
    }//end of presentationIndex
